...
python3 -m task.t2.input_api_based.py
"""
...
# Create AzureChatOpenAI client and UserClient
...
class SearchRequest(BaseModel):
    search_field: SearchField = Field(..., description="Field to search on: name, surname or email")
...
def _join_context(context: list[dict[str, Any]]) -> str:
    """Format a list of user dictionaries into a human-readable string (similar to no_grounding.join_context)."""
    parts: list[str] = []
    for user in context:
        lines = ["User:"]
...
def retrieve_context(user_question: str) -> list[dict[str, Any]]:
    ...
    # 2. Create messages array with system prompt template and user message
    system_template = SystemMessagePromptTemplate.from_template(QUERY_ANALYSIS_PROMPT)
    messages = [system_template, HumanMessage(content=user_question)]

    # 3. Generate `prompt` and partially apply format_instructions
    prompt = ChatPromptTemplate.from_messages(messages=messages).partial(format_instructions=parser.get_format_instructions())

    # 4. Invoke it: (prompt | llm_client | parser).invoke({}) as `search_requests: SearchRequests`
    try:
        # LCEL pipeline
        result = (prompt | llm_client | parser).invoke({})
    except Exception as e:
        print(f"LLM invocation failed: {e}")
        return []

    # result is SearchRequests
    if not result or not hasattr(result, "search_request_parameters"):
        print("No specific search parameters found!")
        return []

    search_requests: SearchRequests = result

    if search_requests.search_request_parameters:
        requests_dict: dict[str, Any] = {}
...
def augment_prompt(user_question: str, context: list[dict[str, Any]]) -> str:
    ...
    # 1. Prepare context from users JSONs in the same way as in `no_grounding.py` `join_context` method
